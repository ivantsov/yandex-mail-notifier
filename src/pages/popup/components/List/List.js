...
function Placeholders({show, count}) {
  const elements = Array.from(Array(count).keys()).map(index => (
...
function List({items, onActionClick, openMessage}) {
  const messages = items.map(item => (
    ...
    >
      <Item onActionClick={onActionClick} openMessage={openMessage} {...item} />
    </CSSTransition>
  ));

  return <TransitionGroup>{messages}</TransitionGroup>;
}
List.propTypes = {
...
const ListContainer = ({
  ...
  return (
    <div className={styles.content}>
      <Placeholders show={loading} count={Math.min(unreadMessagesCount, 100)} />
      {!loading && (
        <List
          items={items}
