...
class App extends Component {
  ...
  componentDidMount() {
    const {user, loadMessages} = this.props;
  ...
  render() {
    const {
      user,
      messages: {unreadCount, items, loading, error},
      settings: {preferredDomain},
      loadMessages,
      updateMessage,
      openLink,
      openSettings,
    ...
    if (!user.authorized) {
      return (
        <UnavailableMessage domain={preferredDomain} reloadApp={reloadApp} />
      );
    }
