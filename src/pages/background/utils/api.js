...
import {getUid as getCookieUid, getSessionId} from './cookie';
...
chrome.webRequest.onBeforeSendHeaders.addListener(
  ...
  {
    urls: appConfig.supportedDomains.map(domain =>
      `${API_URL}/*`.replace('{domain}', domain),
    ),
  },
...
async function sendRequest(data) {
  const {method = 'get', url, type = 'json', form, query} = data;
...
async function loadUserInfo() {
  ...
  const {default_uid: uid, accounts} = await sendRequest({
