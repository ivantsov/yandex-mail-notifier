...
const messageElements = messages.map(
  ({id, subject, from, firstline, date, fid}) =>
    createElement(
      'message',
      [
        createElement('from', [
        ...
        createElement('subject', [createElement('text', [], subject)]),
  ...
  describe('bad xml', () => {
    it('messages', () => {
      ...
      expect(() => parseXML(xml)).toThrowError(
        'Bad response format in messages xml',
      );
    });
    ...
    it('folders', () => {
      ...
      expect(() => parseXML(xml)).toThrowError(
        'Bad response format in folders xml',
      );
    });
    ...
    it('error', () => {
      ...
      expect(() => parseXML(xml)).toThrowError(
        `Messages xml has error field with code: ${errorCode}`,
      );
    });
  });
