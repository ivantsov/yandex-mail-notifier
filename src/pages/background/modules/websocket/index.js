...
import {RECONNECT, MESSAGE} from './constants';
...
async function connect() {
  ...
  const {authorized, token, uid} = getState().user;

  alarms.clear(config.reconnect.name);

  try {
    alarms.create(config.reconnect.name, {
      delayInMinutes: config.reconnect.time,
    });
